...
nrow(accn_info)
SEED_GWAS_GID <- read.table(file = paste(extdata, "SEED_GWAS_GID.tab", sep = "/"),
                            header = TRUE, quote = "",sep ="\t")

# There are one or more GWAS seed identifiers per general identifier in the table

GWAS_ID <- read.table(file = file.path(data, "GWAS_ID.tab"), quote = "",sep ="\t")
...
genotype <- read.table("S3_8542287.tab", header = TRUE, sep ="\t") %>%
  # dplyr::inner_join(read.table("S4_5387180.tab", header = TRUE, sep ="\t"))   %>%
  dplyr::inner_join(read.table("S5_195455216.tab", header = TRUE, sep ="\t"))
...
ele_fr  <- mex_info %>% 
  ...
  unique()%>% dplyr::left_join(soil_bin) %>%
  dplyr::select(-HiVH) %>%
...
lpcat_plot <- ele_fr %>% 
  ggpubr::ggscatter(  x = "Elevation", y = "S5_195455216_fr",
                               add = "loess",               # Add regression line
                               conf.int = TRUE,                # Add confidence interval
                               color = "#1260ad") +
...
pdf("fig1_CD.pdf")
ggpubr::ggarrange(pla1_plot,pcat_plot) %>% print()
dev.off()
```
