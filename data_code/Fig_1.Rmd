...
gmex<-filter(all_pops_pbe,CHROM==3,POS>8.4 ,POS<8.81,POP=="MH") %>%
  ggplot(aes(y=PBE0,x=POS))+
  geom_point(size=1, alpha = 0.9) +
  ylim(-0.5,1.5) +
  xlim(8.4, 8.8) +
...
gswu<-filter(all_pops_pbe,CHROM==8,POS>123.027 ,POS<123.037,POP=="US") %>%
  ...
  theme(axis.title.x=element_blank(),
        ...
        axis.ticks.x=element_blank()) +
  geom_hline(yintercept=0.4, color="red", linetype="dashed") +
  geom_vline(xintercept = 123.034514,col="red",lty=1) +
  geom_vline(xintercept = 123.033420,col="blue",lty=1) +
...
hilo_lipids %>%
  filter (FIELD == "MT2016") %>%
  #filter(PCs/LPCs > 5) %>%
  ggplot(aes(x = log10(PCs), y = log10(LPCs), color = ele_cont)) +
...
Cran <- lapply(1:r, function(x){
  m <- sample.int(2,n*4,replace = TRUE) -1 %>% matrix( nrow = n, ncol = 4)
  colnames(m) <- c("US","MH","GH","AN")
  data.frame( n = x, Cx = single_c_hyper(m[,"US"], m[,"MH"], na.rm = F))
}) %>% dplyr::bind_rows()

hist(Cran$Cx, main =paste("r = ", r))
```

```{r}
outlier_file <- "SuppplementaryFile1.csv"
...
```{r}
single_c_hyper(pbe[,"US"], pbe[,"MH"], na.rm = F)
```

```{r}
single_p_hyper(pbe[,"US"], pbe[,"MH"], na.rm = F)
```

```{r}

single_c_hyper(pbe[,"MH"], pbe[,"GH"], na.rm = F)
```


```{r}
single_p_hyper(pbe[,"MH"], pbe[,"GH"], na.rm = F)
```


```{r}
single_c_hyper(pbe[,"US"], pbe[,"AN"], na.rm = F)
```


```{r}
single_p_hyper(pbe[,"US"], pbe[,"AN"], na.rm = F)
```


```{r}
single_c_hyper(pbe[,"MH"], pbe[,"AN"], na.rm = F)
```


```{r}
single_p_hyper(pbe[,"MH"], pbe[,"AN"], na.rm = F)
```
```{r}
single_c_hyper(pbe[,"GH"], pbe[,"AN"], na.rm = F)
```

```{r}
single_p_hyper(pbe[,"GH"], pbe[,"AN"], na.rm = F)
```
...
```{r}
estimate_pa(pbe, ndigits = 4, show.plot = F, na.rm = F)
```
