...
pla_ionomics <- read_csv("data/Supp_fig8/pla_ionomics.csv")

pla_lipid_phyt <- read_csv("data/Supp_fig8/pla_phyt_20.csv")

pv18_ionomics <- read_csv("data/Supp_fig8/PV18_ionomics.csv")
```
...
pla_ionomics$Genotype = forcats::fct_rev(factor(pla_ionomics$Genotype))

plaP31 <- pla_ionomics %>%
  ggplot(aes(x = Genotype, y = (P31 * 10000) , color = Genotype)) +
  geom_jitter(position=position_jitter(0.1), size = 2) +
...
```{r}
andosols <- read_csv("data/Supp_fig8/andosol_elevation_bins.csv")
