...
```{r}
exp_valuesPLA1 <- read_csv("data/fig_3/ZmPLA1_exp.csv")

PLA1_exp_fig <- exp_valuesPLA1 %>%
	filter(tissue_key=="0") %>%
...
```
```{r}
exp_valuesLPCAT <- read_csv("data/fig_3/ZmLPCAT1_exp.csv")

LPCAT_exp_fig <- exp_valuesLPCAT %>%
	filter(tissue_key=="0") %>%
  #filter(line=="B73"|line=="PT") %>%
  filter(fertilizer_tx=="high") %>%
	ggplot(aes(x = temp_tx, y = ZmLpcat1, color = temp_tx)) +
  scale_color_manual(values=c("#1260ad","#df1b23", "#fd6820", "#14ade8")) +
	geom_jitter(position=position_jitter(0.1), size = 3) +
  stat_summary(fun.data=mean_sdl,fun.args = list(mult = 1), geom="pointrange",
               color = "black") +
...
plot_grid(PLA1_exp_fig, LPCAT_exp_fig, ncol = 1)
