...
metabolite <- read.csv(file.path(data_dir,"pla_phyt_20.csv")) %>%
  ...
  dplyr::rename(sample = "sample_id",
                genotype = "geno") %>%
  dplyr::mutate(genotype = factor(genotype, levels =  c("wt","het","mut"))) %>%
  janitor::clean_names(case = "all_caps") 

colnames(metabolite)[1:3] <- colnames(metabolite)[1:3] %>% tolower()
...
MS2020 <- read.csv(file.path(data_dir,"pla_phyt_20.csv") %>%
  dplyr::mutate(geno = gsub("hom", "mut", geno)) %>%
  dplyr::rename(sample = "sample_id",
                genotype = "geno") %>%
  janitor::clean_names(case = "all_caps")
...
MS2021 <- read.csv(file.path(data_dir,"2021-lipids.csv") %>%
  janitor::clean_names(case = "all_caps")

colnames(MS2021)[1:2] <- colnames(MS2021)[1:2] %>% tolower()
...
CRISPR_norm <- dplyr::left_join( 
  ...
  dplyr::mutate(
    trait = case_when((grepl("^PC_",species)) ~ "PCs",
                      (grepl("LPC_",species)) ~ "LPCs")) %>%
...
paired <-  rbind(CRISPR_norm, RILs_norm) %>%
  ...
  dplyr::filter( n == 2) %>% 
  dplyr::mutate( sign = sign(signalZ[2] - signalZ[1])) %>% print(n=60)
...
paired$label[paired$species == "PC_36_2" & paired$genotype == "mut"] <- "PC36:2"
paired$label[paired$species == "PC_36_5" & paired$genotype == "mut"] <- "PC36:5"
paired$label[paired$species == "LPC_18_2" & paired$genotype == "mut"] <- "LPC18:2"
paired$label[paired$species == "LPC_18_1" & paired$genotype == "mut"] <- "LPC18:1"
...
pe21 <- paired %>%
  ...
  ggpubr::stat_pvalue_manual(
    wilcox_1 %>% dplyr::filter(trait == "PCs"), label = "{pval_fmt}", bracket.nudge.y = 0.1,
  ) +
...
pe22 <- paired %>%
  ...
  # ggplot2::facet_wrap(pop ~ trait, scales = "free_y") +
  ggpubr::stat_pvalue_manual(
    wilcox_1 %>% dplyr::filter(trait == "LPCs"), label = "{pval_fmt}", bracket.nudge.y = 0.1,
  ) +
  ggrepel::geom_text_repel(
    data= paired  %>%
...
pe23 <- paired %>%
  ...
  # ggplot2::facet_wrap(pop ~ trait, scales = "free_y") +
  ggpubr::stat_pvalue_manual(
    wilcox_2 %>% dplyr::filter(trait == "PCs"), label = "{pval_fmt}", bracket.nudge.y = 0.1,
  ) +
  ggrepel::geom_text_repel(
    data= paired  %>%
      dplyr::filter(pop == "CRISPR/CAS9" & species %in% c("PC_36_2","PC_36_5")),
    aes(label = label),
    hjust = 2,
    min.segment.length = unit(0, 'lines'),
    segment.size = 0.5,
...
pe24 <- paired %>%
  ...
  ggplot2::ggplot(aes( x = genotype, y = signalZ, group = species, color = factor(sign))) +
  ggplot2::geom_point() + geom_line() +
  ggplot2::coord_cartesian( ylim = c(-0.75, 1.25), 
                            xlim = c(1,2.5))  +
  ggplot2::ylab("Signal (Z-score)") + ggplot2::xlab("Genotype")   +
  ggplot2::scale_color_manual(values= c("grey50","black") ) +
  ggplot2::scale_x_discrete(labels = c(expression(italic("wt")), expression(italic("CR")))) +
  ...
  # ggplot2::facet_wrap(pop ~ trait, scales = "free_y") +
  ggpubr::stat_pvalue_manual(
    wilcox_2 %>% dplyr::filter(trait == "LPCs"), label = "{pval_fmt}", bracket.nudge.y = 0.1,
  ) +
  ggrepel::geom_text_repel(
    data= paired  %>%
      dplyr::filter(pop == "CRISPR/CAS9" & species %in% c("LPC_18_1","LPC_18_2")),
    aes(label = label),
    hjust = 2,
    min.segment.length = unit(0, 'lines'),
    segment.size = 0.5,
