...
%\title{Adaptive teosinte mexicana introgression of \textit{HPC1} in highland maize and modern inbreds controls phosphatidylcholine levels and reduces flowering time}
%\title{Adaptive teosinte mexicana introgression of \textit{HPC1} in highland maize and Northern Flints controls phosphatidylcholine levels and reduces flowering time}
%\title{Adaptive teosinte \textit{mexicana} introgression of \textit{HPC1} in highland maize controls phosphatidylcholine levels and reduces flowering time}
%\title{Adaptive highland teosinte introgression into maize at \textit{HPC1} controls phosphatidylcholine levels and induces earlier flowering} %Thanks Matt!
%\title{Adaptive introgression at \textit{High PhosphatidylCholine 1} controls phosphatidylcholine levels and induces earlier flowering in maize}
%\title{Adaptive introgression at \textit{High PhosphatidylCholine 1} modulates phospholipid levels and induces early flowering in maize}
...
Previous work has linked a modification of phospholipid metabolism to responses to low phosphorous and low temperature stress and to changes in flowering time.
...
Common garden experiments demonstrated strong genotype by environment interaction associated with variation at \textit{HPC1}, the highland allele of \textit{HPC1} leading to higher fitness possibly via accelerated flowering. The variant of \textit{HPC1} we identified in highland maize is a putative loss of function allele carrying a substitution in a highly conserved amino acid. A meta-analysis of bacterial growth in different environments indicated a conserved association between this amino acid and temperature. CRISPR-CAS9 mutagenesis of \textit{HPC1} confirmed its role in regulating phospholipid metabolism and flowering time. Finally, we show that the highland \textit{HPC1} allele entered cultivated maize by introgression from the wild highland teosinte \textit{Zea mays ssp. mexicana} and has been conserved in Northern US and European maize breeding lines.
\end{abstract}
...
Following domestication from teosinte parviglumis (\textit{Zea mays} spp. \textit{parviglumis}) \cite{Matsuoka2002-bg,Piperno2009-fj} in the lowland, subtropical environment of the Balsas River (Guerrero, México)% around 9,000 BP
...
% Fausto: maybe we can simplify GGDU to low temperatures during the growing season?
%Jeff: maybe move this to \P above?
...
On the other hand, lyso-phosphatidylcholine (LPC), which is a PC with a single fatty acid, is a non-bilayer-forming triangular shaped lipid because its headgroup much larger than its fatty acid \cite{Jouhet2013-fv}.
LPCs allow for some membrane movement but high LPC concentration acts as a detergent \cite{Henriksen2010-cm} and can facilitate cell leakage and damage in stressful conditions while higher PC levels would prevent this type of damage.
...
Recent studies \cite{Wang2020-mp, Crow2020-gene} have improved our understanding of the genetics of maize highland adaptation and the role of highland teosinte mexicana in this process \cite{Hufford2013-gs, Lauter2004-eq, pyhajarvi2013}.
...
Here, we connect an adaptive teosinte \textit{mexicana} introgression to changes in highland maize glycerolipid metabolism.
%TODO Allison and Fausto edit this section - 1 - How (physiologically, metabolically) did maize adapt to highlands? 2 - What was the role of Teosinte in this process? What is the knowledge gap we are addressing.

%RS this summary paragraph may be a little over long
...
Using data from thousands of genotyped landrace test-crosses grown in common garden experiments at different elevations in México, we showed a strong genotype by environment effect of the \textit{HPC1} locus, 
where the highland allele leads to higher fitness in highlands and reduced fitness at lower elevations. 
...
\end{center}
\end{figure*} 
\textit{ZmLPCAT1} is an example of a gene selected, with shared outlier SNPs in the CDS region, in all four populations that is part of the 'phospholipid remodelling' and 'PC acyl editing' pathways (Figure \ref{Fig2}E), Supplementary File 1). %Jeff: awkward sentence. a rewrite for brevity/clarity? split into two?
...
%Variants located in the \textit{HPC1} CDS \cite{Chen2012-gg} and 7 Kb upstream  of the gene \cite{Hung2012-ms} show significant associations with flowering time in the Nested Association Mapping population. 
%\textit{HPC1} also showed significant associations with glycerolipid content variation in a maize lipid GWAS analysis using the European Flint and Dent inbred panels \cite{Riedelsheimer2013-bd}.
%Taken together the phospholipid pathways selectshow clear signs of recurrent selection across several highland populations both at the metabolic (PC/LPC ratios) and gene level such as \textit{HPC1} and \textit{ZmLPCAT1}.   
...
% RS "typically exposed to 5 growth degree units across the growth season" clarify, out in context
...
%In B73, \textit{HPC1} is one of the most highly expressed phospholipases and its expression pattern is almost restricted to vegetative leaves (V4-V9) (Supplementary Figure\ref{SupFig4}A) \cite{Stelpflug2016-vr}, the same type of leaves that we sampled for glycerolipid analysis. 
...
%\textit{HPC1} is highly expressed in B73 and other temperate inbreds under low temperature conditions and is downregulated in heat conditions (Supplementary Figure\ref{SupFig4}C) \cite{Waters2017-nat}.
...
%The ratio of PC to LPC is higher in BILs that are homozygous PT at the \textit{HPC1} locus. 
...
%Furthermore, the QTL data suggest that the highland PT allele is a loss of function of \textit{HPC1}. 
...
%We are currently performing further experiments in conditions simulating highland environments to test if the \textit{HPC1\textsuperscript{CR}} mutant shows a similar reduction in flowering time as what we observed with in Figure \ref{Fig4}A, confirming an interaction between \textit{HPC1} and environment.


%\textbf{Association of \textit{HPC1\textsuperscript{CR}} with phosphorus levels}.
...
%To test if \textit{HPC1} is associated with phosphorus leaf levels we measured phosphorus content in leaves \textit{HPC1} CRISPR-CAS9 mutants but we did not observe any differences when compared to wild type plants(Supplementary Fig \ref{SupFig6}C.) 
...
%We then investigated the possible effects of genetic variation in \textit{HPC1-PT} alleles on several fitness traits to explore the relevance of genetic variation at \textit{HPC1-PT} in maize local adaptation to highland conditions.
 \section{Discussion}
...
%SNPs in the coding regions of \textit{ZmLPCAT1} and \textit{HPC1} genes were also strong outliers in the first \textit{pcadapt} principal components (that captures elevation dependent genetic variation) analysis of thousands of accessions across México (Figure \ref{Fig2} and Supplementary \ref{SupFig2}. 
...
Conversely, the previously characterized \textit{mexicana} introgression \textit{inv4m} \cite{Gonzalez-Segovia2019-jy, pyhajarvi2013, Hufford2013-gs} shows no effect in highlands and negative effect in lowlands \cite{Crow2020-gene}.
